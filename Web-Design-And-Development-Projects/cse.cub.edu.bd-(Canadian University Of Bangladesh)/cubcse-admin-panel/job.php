...
<div class="container-fluid">
  ...
  <div id="admin-content">
      <div class="container">
          <div class="row">
              <div class="col-md-10">
                  <h1 class="admin-heading">All job Lists</h1>
              </div>
              <div class="col-md-2">
                  <a class="add-new" href="add-job.php"><button class="btn">Add Job</button></a>
              </div>
              <div class="col-md-12">
<br>
<?php 
...
if($_SESSION['user_role'] == '1'){
   $query = "SELECT job.job_id, job.name, job.link, job.job_img, job.job_date,job.category, category.category_name,user.username FROM job
  ...
  LEFT JOIN category ON job.category = category.category_id
  LEFT JOIN user ON job.author = user.user_id
  WHERE job.author = {$_SESSION['user_id']}
  ORDER BY job.job_id DESC LIMIT {$offset },{$limit}";
}
...
?>
                          <tr>
                              ...
                              <td><img height="50px" src="upload/<?php echo $row['job_img'] ?>"></td>
                              <td><?php echo $row['name'] ?></td>
                              <td><?php echo $row['category_name'] ?></td>
                              <td><?php echo $row['job_date'] ?></td>
                              <td><?php echo $row['username'] ?></td>

<td class='edit'><a href='update-job.php?id=<?php echo $row['job_id'] ?>'><i class='fa fa-edit'></i></a></td>
<td class='delete'><a onclick="return confirm('Are You Sure?')" href='delete-job.php?id=<?php echo $row['job_id'] ?>&catid=<?php echo $row['category'] ?>'><i class='fa fa-trash-o'></i></a></td>
...
<?php 
      ...
      if(mysqli_num_rows($result2)){
        ...
        if($page_number > 1){
          echo '<li><a href="job.php?page='.($page_number-1).'">prev</a></li>';
        }
        ...
        for($i = 1; $i <= $total_page; $i++){
          ...
          echo '<li class='.$active.'><a href="job.php?page='.$i.'">'.$i.'</a></li>';
        }
        if($total_page > $page_number){
          echo '<li><a href="job.php?page='.($page_number+1).'">next</a></li>';
        }
        echo "</ul>";
      }
