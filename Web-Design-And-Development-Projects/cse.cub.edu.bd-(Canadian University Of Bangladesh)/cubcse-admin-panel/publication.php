...
<div class="container-fluid">
  ...
  <div id="admin-content">
      <div class="container">
          <div class="row">
              <div class="col-md-10">
                  <h1 class="admin-heading">All Publication</h1>
              </div>
              <div class="col-md-2">
                  <a class="add-new" href="add-publication.php"><button class="btn">Add Publication</button></a>
              </div>
              <div class="col-md-12">
<br>
<?php 
...
if($_SESSION['user_role'] == '1'){
   $query = "SELECT publication.publication_id, publication.title, publication.description, publication.link, publication.position, publication.publication_date,publication.category, category.category_name,user.username FROM publication
  ...
  LEFT JOIN category ON publication.category = category.category_id
  LEFT JOIN user ON publication.author = user.user_id
  WHERE publication.author = {$_SESSION['user_id']}
  ORDER BY publication.position ASC LIMIT {$offset },{$limit}";
}
...
<td class='edit'><a href='update-publication.php?id=<?php echo $row['publication_id'] ?>'><i class='fa fa-edit'></i></a></td>
<td class='delete'><a onclick="return confirm('Are You Sure?')" href='delete-publication.php?id=<?php echo $row['publication_id'] ?>&catid=<?php echo $row['category'] ?>'><i class='fa fa-trash-o'></i></a></td>
...
<?php 
      ...
      include "config.php";
      $query2 = "SELECT * FROM publication";
      ...
      if(mysqli_num_rows($result2)){
        ...
        if($page_number > 1){
          echo '<li><a href="publication.php?page='.($page_number-1).'">prev</a></li>';
        }
        ...
        for($i = 1; $i <= $total_page; $i++){
          ...
          echo '<li class='.$active.'><a href="publication.php?page='.$i.'">'.$i.'</a></li>';
        }
        if($total_page > $page_number){
          echo '<li><a href="publication.php?page='.($page_number+1).'">next</a></li>';
        }
        echo "</ul>";
      }
