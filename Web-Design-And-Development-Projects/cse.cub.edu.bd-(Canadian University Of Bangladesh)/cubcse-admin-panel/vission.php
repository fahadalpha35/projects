...
<div class="container-fluid">
  ...
  <div id="admin-content">
      <div class="container">
          <div class="row">
              <div class="col-md-10">
                  <h1 class="admin-heading">Vissions</h1>
              </div>
              <div class="col-md-2">
                  <a class="add-new" href="add-vission.php"><button class="btn">Add Vission</button></a>
              </div>
              <div class="col-md-12">
<br>
<?php 
...
if($_SESSION['user_role'] == '1'){
   $query = "SELECT vission.vission_id, vission.description, vission.vission_date,vission.category, category.category_name,user.username FROM vission
  ...
  LEFT JOIN category ON vission.category = category.category_id
  LEFT JOIN user ON vission.author = user.user_id
  WHERE vission.author = {$_SESSION['user_id']}
  ORDER BY vission.vission_id DESC LIMIT {$offset },{$limit}";
}
...
?>
                          <tr>
                              ...
                              <td><?php echo $row['vission_date'] ?></td>
                              <td><?php echo $row['username'] ?></td>

<td class='edit'><a href='update-vission.php?id=<?php echo $row['vission_id'] ?>'><i class='fa fa-edit'></i></a></td>
<td class='delete'><a onclick="return confirm('Are You Sure?')" href='delete-vission.php?id=<?php echo $row['vission_id'] ?>&catid=<?php echo $row['category'] ?>'><i class='fa fa-trash-o'></i></a></td>
...
<?php 
      ...
      if(mysqli_num_rows($result2)){
        ...
        if($page_number > 1){
          echo '<li><a href="vission.php?page='.($page_number-1).'">prev</a></li>';
        }
        ...
        for($i = 1; $i <= $total_page; $i++){
          ...
          echo '<li class='.$active.'><a href="vission.php?page='.$i.'">'.$i.'</a></li>';
        }
        if($total_page > $page_number){
          echo '<li><a href="vission.php?page='.($page_number+1).'">next</a></li>';
        }
        echo "</ul>";
      }
...
?></center> 
			...
			<div class="col-md-10">
                  <h1 class="admin-heading">Missions</h1>
              </div>
              <div class="col-md-2">
                  <a class="add-new" href="add-mission.php"><button class="btn">Add Mission</button></a>
              </div>  
...
if($_SESSION['user_role'] == '1'){
   $query = "SELECT mission.mission_id, mission.description, mission.mission_date,mission.category, category.category_name,user.username FROM mission
  ...
  LEFT JOIN category ON mission.category = category.category_id
  LEFT JOIN user ON mission.author = user.user_id
  WHERE mission.author = {$_SESSION['user_id']}
  ORDER BY mission.mission_id DESC LIMIT {$offset },{$limit}";
}
...
<td class='edit'><a href='update-mission.php?id=<?php echo $row['mission_id'] ?>'><i class='fa fa-edit'></i></a></td>
<td class='delete'><a onclick="return confirm('Are You Sure?')" href='delete-mission.php?id=<?php echo $row['mission_id'] ?>&catid=<?php echo $row['category'] ?>'><i class='fa fa-trash-o'></i></a></td>
