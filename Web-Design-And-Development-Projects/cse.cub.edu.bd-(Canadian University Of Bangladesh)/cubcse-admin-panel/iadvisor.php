...
<div class="container-fluid">
  ...
  <div id="admin-content">
      <div class="container">
          <div class="row">
              <div class="col-md-10">
                  <h1 class="admin-heading">All Internation Advisor Lists</h1>
              </div>
              <div class="col-md-2">
                  <a class="add-new" href="add-iadvisor.php"><button class="btn">Add Member</button></a>
              </div>
              <div class="col-md-12">
<br>
<?php 
...
if($_SESSION['user_role'] == '1'){
   $query = "SELECT iadvisor.iadvisor_id, iadvisor.title, iadvisor.designation, iadvisor.email, iadvisor.iadvisor_img, iadvisor.position, iadvisor.iadvisor_date,iadvisor.category, category.category_name,user.username FROM iadvisor
  ...
  LEFT JOIN category ON iadvisor.category = category.category_id
  LEFT JOIN user ON iadvisor.author = user.user_id
  WHERE iadvisor.author = {$_SESSION['user_id']}
  ORDER BY iadvisor.position ASC LIMIT {$offset },{$limit}";
}
...
?>
                          <tr>
                              ...
                              <td><img height="50px" src="upload/<?php echo $row['iadvisor_img'] ?>"></td>
                              <td><?php echo $row['title'] ?></td>
							  <td><?php echo $row['position'] ?></td>
                              <td><?php echo $row['category_name'] ?></td>
                              <td><?php echo $row['iadvisor_date'] ?></td>
                              <td><?php echo $row['username'] ?></td>

<td class='edit'><a href='update-iadvisor.php?id=<?php echo $row['iadvisor_id'] ?>'><i class='fa fa-edit'></i></a></td>
<td class='delete'><a onclick="return confirm('Are You Sure?')" href='delete-iadvisor.php?id=<?php echo $row['iadvisor_id'] ?>&catid=<?php echo $row['category'] ?>'><i class='fa fa-trash-o'></i></a></td>
...
<?php 
      ...
      if(mysqli_num_rows($result2)){
        ...
        if($page_number > 1){
          echo '<li><a href="iadvisor.php?page='.($page_number-1).'">prev</a></li>';
        }
        ...
        for($i = 1; $i <= $total_page; $i++){
          ...
          echo '<li class='.$active.'><a href="iadvisor.php?page='.$i.'">'.$i.'</a></li>';
        }
        if($total_page > $page_number){
          echo '<li><a href="iadvisor.php?page='.($page_number+1).'">next</a></li>';
        }
        echo "</ul>";
      }
