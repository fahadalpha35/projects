...
<div class="container-fluid">
  ...
  <div id="admin-content">
      <div class="container">
          <div class="row">
              <div class="col-md-10">
                  <h1 class="admin-heading">Admission Notice</h1>
              </div>
              <div class="col-md-12">
<br>
<?php 
...
if($_SESSION['user_role'] == '1'){
   $query = "SELECT message.message_id, message.description, message.message_date,message.category, category.category_name,user.username FROM message
  ...
  LEFT JOIN category ON message.category = category.category_id
  LEFT JOIN user ON message.author = user.user_id
  WHERE message.author = {$_SESSION['user_id']}
  ORDER BY message.message_id ASC LIMIT {$offset },{$limit}";
}
...
<td class='edit'><a href='update-message.php?id=<?php echo $row['message_id'] ?>'><i class='fa fa-edit'></i></a></td>
...
<?php 
      ...
      if(mysqli_num_rows($result2)){
        ...
        if($page_number > 1){
          echo '<li><a href="message.php?page='.($page_number-1).'">prev</a></li>';
        }
        ...
        if($total_page > $page_number){
          echo '<li><a href="message.php?page='.($page_number+1).'">next</a></li>';
        }
        echo "</ul>";
      }
