...
<div class="container-fluid">
  ...
  <div id="admin-content">
      <div class="container">
          <div class="row">
              <div class="col-md-10">
                  <h1 class="admin-heading">All Administration Lists</h1>
              </div>
              <div class="col-md-2">
                  <a class="add-new" href="add-administration.php"><button class="btn">Add Member</button></a>
              </div>
              <div class="col-md-12">
<br>
<?php 
...
if($_SESSION['user_role'] == '1'){
   $query = "SELECT administration.admin_id, administration.title, administration.designation, administration.email, administration.admin_img, administration.position, administration.admin_date,administration.category, category.category_name,user.username FROM administration
  ...
  LEFT JOIN category ON administration.category = category.category_id
  LEFT JOIN user ON administration.author = user.user_id
  WHERE administration.author = {$_SESSION['user_id']}
  ORDER BY administration.position ASC LIMIT {$offset },{$limit}";
}
...
?>
                          <tr>
                              ...
                              <td><img height="50px" src="upload/<?php echo $row['admin_img'] ?>"></td>
...
<td class='edit'><a href='update-administration.php?id=<?php echo $row['admin_id'] ?>'><i class='fa fa-edit'></i></a></td>
<td class='delete'><a onclick="return confirm('Are You Sure?')" href='delete-administration.php?id=<?php echo $row['admin_id'] ?>&catid=<?php echo $row['category'] ?>'><i class='fa fa-trash-o'></i></a></td>
...
<?php 
      ...
      include "config.php";
      $query2 = "SELECT * FROM administration";
      ...
      if(mysqli_num_rows($result2)){
        ...
        if($page_number > 1){
          echo '<li><a href="administration.php?page='.($page_number-1).'">prev</a></li>';
        }
        ...
        for($i = 1; $i <= $total_page; $i++){
          ...
          echo '<li class='.$active.'><a href="administration.php?page='.$i.'">'.$i.'</a></li>';
        }
        if($total_page > $page_number){
          echo '<li><a href="administration.php?page='.($page_number+1).'">next</a></li>';
        }
        echo "</ul>";
      }
