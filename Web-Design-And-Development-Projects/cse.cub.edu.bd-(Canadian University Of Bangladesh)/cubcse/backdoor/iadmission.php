...
<div class="container-fluid">
  ...
  <div id="admin-content">
      <div class="container">
          <div class="row">
              <div class="col-md-10">
                  <h1 class="admin-heading">All International Admission Requirements</h1>
              </div>
              <div class="col-md-2">
                  <a class="add-new" href="add-iadmission.php"><button class="btn">Add iadmission</button></a>
              </div>
              <div class="col-md-12">
<br>
<?php 
...
if($_SESSION['user_role'] == '1'){
   $query = "SELECT iadmission.iadmission_id, iadmission.description, iadmission.iadmission_date,iadmission.category, category.category_name,user.username FROM iadmission
  ...
  LEFT JOIN category ON iadmission.category = category.category_id
  LEFT JOIN user ON iadmission.author = user.user_id
  WHERE iadmission.author = {$_SESSION['user_id']}
  ORDER BY iadmission.iadmission_id ASC LIMIT {$offset },{$limit}";
}
...
?>
                          <tr>
                              ...
                              <td><?php echo $row['iadmission_date'] ?></td>
                              <td><?php echo $row['username'] ?></td>

<td class='edit'><a href='update-iadmission.php?id=<?php echo $row['iadmission_id'] ?>'><i class='fa fa-edit'></i></a></td>
<td class='delete'><a onclick="return confirm('Are You Sure?')" href='delete-iadmission.php?id=<?php echo $row['iadmission_id'] ?>&catid=<?php echo $row['category'] ?>'><i class='fa fa-trash-o'></i></a></td>
...
<?php 
      ...
      if(mysqli_num_rows($result2)){
        ...
        if($page_number > 1){
          echo '<li><a href="iadmission.php?page='.($page_number-1).'">prev</a></li>';
        }
        ...
        for($i = 1; $i <= $total_page; $i++){
          ...
          echo '<li class='.$active.'><a href="iadmission.php?page='.$i.'">'.$i.'</a></li>';
        }
        if($total_page > $page_number){
          echo '<li><a href="iadmission.php?page='.($page_number+1).'">next</a></li>';
        }
        echo "</ul>";
      }
