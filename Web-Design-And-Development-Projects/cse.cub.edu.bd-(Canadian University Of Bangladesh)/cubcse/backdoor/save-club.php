<?php
  ...
  $title7 = mysqli_real_escape_string($connection, $_POST['post_title7']);
  $category = mysqli_real_escape_string($connection, $_POST['category']);
  $date = date("d M, Y");
  $author = $_SESSION['user_id'];

  $sql = "INSERT INTO club(description1, description2,description3,description4,description5,description6,description7, category,club_date,author,club_img)


          VALUES('{$title1}','{$title2}','{$title3}','{$title4}','{$title5}','{$title6}','{$title7}',{$category},'{$date}',{$author},'{$new_name}');";
