...
<div class="container-fluid">
  ...
  <div id="admin-content">
      <div class="container">
          <div class="row">
              <div class="col-md-10">
                  <h1 class="admin-heading">All Club Lists</h1>
              </div>
              <div class="col-md-2">
                  <a class="add-new" href="add-club.php"><button class="btn">Add club</button></a>
              </div>
              <div class="col-md-12">
<br>
<?php 
...
if($_SESSION['user_role'] == '1'){
   $query = "SELECT club.club_id, club.description1, club.description2, club.description3, club.description4, club.description5, club.description6, club.description7, club.club_img, club.club_date,club.category, category.category_name,user.username FROM club
  ...
  LEFT JOIN category ON club.category = category.category_id
  LEFT JOIN user ON club.author = user.user_id
  WHERE club.author = {$_SESSION['user_id']}
  ORDER BY club.club_id DESC LIMIT {$offset },{$limit}";
}
...
?>
                          <tr>
                              ...
                              <td><img height="50px" src="upload/<?php echo $row['club_img'] ?>"></td>
                              <td><?php echo $row['description1'] ?></td>
                              <td><?php echo $row['category_name'] ?></td>
                              <td><?php echo $row['club_date'] ?></td>
                              <td><?php echo $row['username'] ?></td>

<td class='edit'><a href='update-club.php?id=<?php echo $row['club_id'] ?>'><i class='fa fa-edit'></i></a></td>
<td class='delete'><a onclick="return confirm('Are You Sure?')" href='delete-club.php?id=<?php echo $row['club_id'] ?>&catid=<?php echo $row['category'] ?>'><i class='fa fa-trash-o'></i></a></td>
...
<?php 
      ...
      if(mysqli_num_rows($result2)){
        ...
        if($page_number > 1){
          echo '<li><a href="club.php?page='.($page_number-1).'">prev</a></li>';
        }
        ...
        for($i = 1; $i <= $total_page; $i++){
          ...
          echo '<li class='.$active.'><a href="club.php?page='.$i.'">'.$i.'</a></li>';
        }
        if($total_page > $page_number){
          echo '<li><a href="club.php?page='.($page_number+1).'">next</a></li>';
        }
        echo "</ul>";
      }
