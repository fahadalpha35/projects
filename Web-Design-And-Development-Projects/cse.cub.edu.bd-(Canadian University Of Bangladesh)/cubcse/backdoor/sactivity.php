...
<div class="container-fluid">
  ...
  <div id="admin-content">
      <div class="container">
          <div class="row">
              <div class="col-md-10">
                  <h1 class="admin-heading">All Activity Lists</h1>
              </div>
              <div class="col-md-2">
                  <a class="add-new" href="add-activity.php"><button class="btn">Add Activity</button></a>
              </div>
              <div class="col-md-12">
<br>
<?php 
...
if($_SESSION['user_role'] == '1'){
   $query = "SELECT activity.activity_id, activity.description, activity.activity_img, activity.activity_date,activity.category, category.category_name,user.username FROM activity
  ...
  LEFT JOIN category ON activity.category = category.category_id
  LEFT JOIN user ON activity.author = user.user_id
  WHERE activity.author = {$_SESSION['user_id']}
  ORDER BY activity.activity_id DESC LIMIT {$offset },{$limit}";
}
...
?>
                          <tr>
                              ...
                              <td><img height="50px" src="upload/<?php echo $row['activity_img'] ?>"></td>
                              <td><?php echo $row['description'] ?></td>
                              <td><?php echo $row['category_name'] ?></td>
                              <td><?php echo $row['activity_date'] ?></td>
                              <td><?php echo $row['username'] ?></td>

<td class='edit'><a href='update-activity.php?id=<?php echo $row['activity_id'] ?>'><i class='fa fa-edit'></i></a></td>
<td class='delete'><a onclick="return confirm('Are You Sure?')" href='delete-activity.php?id=<?php echo $row['activity_id'] ?>&catid=<?php echo $row['category'] ?>'><i class='fa fa-trash-o'></i></a></td>
...
<?php 
      ...
      if(mysqli_num_rows($result2)){
        ...
        if($page_number > 1){
          echo '<li><a href="activity.php?page='.($page_number-1).'">prev</a></li>';
        }
        ...
        if($total_page > $page_number){
          echo '<li><a href="activity.php?page='.($page_number+1).'">next</a></li>';
        }
        echo "</ul>";
      }
