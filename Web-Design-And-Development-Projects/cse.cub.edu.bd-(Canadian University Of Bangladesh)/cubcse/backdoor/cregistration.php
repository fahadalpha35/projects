...
<div class="container-fluid">
  ...
  <div id="admin-content">
      <div class="container">
          <div class="row">
              <div class="col-md-10">
                  <h1 class="admin-heading">All Course Registration</h1>
              </div>
              <div class="col-md-2">
                  <a class="add-new" href="add-cregistration.php"><button class="btn">Add cregistration</button></a>
              </div>
              <div class="col-md-12">
<br>
<?php 
...
if($_SESSION['user_role'] == '1'){
   $query = "SELECT cregistration.cregistration_id, cregistration.title, cregistration.description,cregistration.cregistration_img, cregistration.cregistration_date,cregistration.category, category.category_name,user.username FROM cregistration
  ...
  LEFT JOIN category ON cregistration.category = category.category_id
  LEFT JOIN user ON cregistration.author = user.user_id
  WHERE cregistration.author = {$_SESSION['user_id']}
  ORDER BY cregistration.cregistration_id DESC LIMIT {$offset },{$limit}";
}
...
?>
                          <tr>
                              ...
                              <td><img height="50px" src="upload/<?php echo $row['cregistration_img'] ?>"></td>
                              <td><?php echo $row['title'] ?></td>
                              <td><?php echo $row['category_name'] ?></td>
                              <td><?php echo $row['cregistration_date'] ?></td>
                              <td><?php echo $row['username'] ?></td>

<td class='edit'><a href='update-cregistration.php?id=<?php echo $row['cregistration_id'] ?>'><i class='fa fa-edit'></i></a></td>
<td class='delete'><a onclick="return confirm('Are You Sure?')" href='delete-cregistration.php?id=<?php echo $row['cregistration_id'] ?>&catid=<?php echo $row['category'] ?>'><i class='fa fa-trash-o'></i></a></td>
...
<?php 
      ...
      include "config.php";
      $query2 = "SELECT * FROM cregistration";
      ...
      if(mysqli_num_rows($result2)){
        ...
        if($page_number > 1){
          echo '<li><a href="cregistration.php?page='.($page_number-1).'">prev</a></li>';
        }
        ...
        for($i = 1; $i <= $total_page; $i++){
          ...
          echo '<li class='.$active.'><a href="cregistration.php?page='.$i.'">'.$i.'</a></li>';
        }
        if($total_page > $page_number){
          echo '<li><a href="cregistration.php?page='.($page_number+1).'">next</a></li>';
        }
        echo "</ul>";
      }
