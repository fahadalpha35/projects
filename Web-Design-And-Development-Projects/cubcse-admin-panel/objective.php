...
<div class="container-fluid">
  ...
  <div id="admin-content">
      <div class="container">
          <div class="row">
              <div class="col-md-10">
                  <h1 class="admin-heading">Objectives</h1>
              </div>
              <div class="col-md-2">
                  <a class="add-new" href="add-objective.php"><button class="btn">Add objective</button></a>
              </div>
              <div class="col-md-12">
<br>
<?php 
...
if($_SESSION['user_role'] == '1'){
   $query = "SELECT objective.objective_id, objective.description, objective.objective_date,objective.category, category.category_name,user.username FROM objective
  ...
  LEFT JOIN category ON objective.category = category.category_id
  LEFT JOIN user ON objective.author = user.user_id
  WHERE objective.author = {$_SESSION['user_id']}
  ORDER BY objective.objective_id DESC LIMIT {$offset },{$limit}";
}
...
?>
                          <tr>
                              ...
                              <td><?php echo $row['objective_date'] ?></td>
                              <td><?php echo $row['username'] ?></td>

<td class='edit'><a href='update-objective.php?id=<?php echo $row['objective_id'] ?>'><i class='fa fa-edit'></i></a></td>
<td class='delete'><a onclick="return confirm('Are You Sure?')" href='delete-objective.php?id=<?php echo $row['objective_id'] ?>&catid=<?php echo $row['category'] ?>'><i class='fa fa-trash-o'></i></a></td>
...
<?php 
      ...
      if(mysqli_num_rows($result2)){
        ...
        if($page_number > 1){
          echo '<li><a href="objective.php?page='.($page_number-1).'">prev</a></li>';
        }
        ...
        for($i = 1; $i <= $total_page; $i++){
          ...
          echo '<li class='.$active.'><a href="objective.php?page='.$i.'">'.$i.'</a></li>';
        }
        if($total_page > $page_number){
          echo '<li><a href="objective.php?page='.($page_number+1).'">next</a></li>';
        }
        echo "</ul>";
      }
...
?></center> 
			...
			<div class="col-md-10">
                  <h1 class="admin-heading">Outcomes</h1>
              </div>
              <div class="col-md-2">
                  <a class="add-new" href="add-outcome.php"><button class="btn">Add Outcome</button></a>
              </div>  
...
if($_SESSION['user_role'] == '1'){
   $query = "SELECT outcome.outcome_id, outcome.description, outcome.outcome_date,outcome.category, category.category_name,user.username FROM outcome
  ...
  LEFT JOIN category ON outcome.category = category.category_id
  LEFT JOIN user ON outcome.author = user.user_id
  WHERE outcome.author = {$_SESSION['user_id']}
  ORDER BY outcome.outcome_id DESC LIMIT {$offset },{$limit}";
}
...
?>
                          <tr>
							  ...
							  <td><?php echo $row['description'] ?></td>
                              <td><?php echo $row['category_name'] ?></td>
                              <td><?php echo $row['outcome_date'] ?></td>
                              <td><?php echo $row['username'] ?></td>

<td class='edit'><a href='update-outcome.php?id=<?php echo $row['outcome_id'] ?>'><i class='fa fa-edit'></i></a></td>
<td class='delete'><a onclick="return confirm('Are You Sure?')" href='delete-outcome.php?id=<?php echo $row['outcome_id'] ?>&catid=<?php echo $row['category'] ?>'><i class='fa fa-trash-o'></i></a></td>
