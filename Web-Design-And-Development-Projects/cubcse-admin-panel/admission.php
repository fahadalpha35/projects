...
<div class="container-fluid">
  ...
  <div id="admin-content">
      <div class="container">
          <div class="row">
              <div class="col-md-10">
                  <h1 class="admin-heading">All Admission Requirements</h1>
              </div>
              <div class="col-md-2">
                  <a class="add-new" href="add-admission.php"><button class="btn">Add admission</button></a>
              </div>
              <div class="col-md-12">
<br>
<?php 
...
if($_SESSION['user_role'] == '1'){
   $query = "SELECT admission.admission_id, admission.description, admission.admission_date,admission.category, category.category_name,user.username FROM admission
  ...
  LEFT JOIN category ON admission.category = category.category_id
  LEFT JOIN user ON admission.author = user.user_id
  WHERE admission.author = {$_SESSION['user_id']}
  ORDER BY admission.admission_id ASC LIMIT {$offset },{$limit}";
}
...
<td class='edit'><a href='update-admission.php?id=<?php echo $row['admission_id'] ?>'><i class='fa fa-edit'></i></a></td>
<td class='delete'><a onclick="return confirm('Are You Sure?')" href='delete-admission.php?id=<?php echo $row['admission_id'] ?>&catid=<?php echo $row['category'] ?>'><i class='fa fa-trash-o'></i></a></td>
...
<?php 
      ...
      if(mysqli_num_rows($result2)){
        ...
        if($page_number > 1){
          echo '<li><a href="admission.php?page='.($page_number-1).'">prev</a></li>';
        }
        ...
        for($i = 1; $i <= $total_page; $i++){
          ...
          echo '<li class='.$active.'><a href="admission.php?page='.$i.'">'.$i.'</a></li>';
        }
        if($total_page > $page_number){
          echo '<li><a href="admission.php?page='.($page_number+1).'">next</a></li>';
        }
        echo "</ul>";
      }
