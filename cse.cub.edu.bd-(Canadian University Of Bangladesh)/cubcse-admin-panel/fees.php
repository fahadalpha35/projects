...
<div class="container-fluid">
  ...
  <div id="admin-content">
      <div class="container">
          <div class="row">
              <div class="col-md-10">
                  <h1 class="admin-heading">Local Student Fee Structure</h1>
              </div>
              <div class="col-md-2">
                  <a class="add-new" href="add-fees.php"><button class="btn">Add Fees</button></a>
              </div>
              <div class="col-md-12">
<br>
<?php 
...
if($_SESSION['user_role'] == '1'){
   $query = "SELECT fees.fees_id, fees.description1, fees.description2, fees.description3, fees.description4, fees.description5, fees.description6, fees.description7, fees.description8, fees.description9, fees.description10, fees.fees_date,fees.category, category.category_name,user.username FROM fees
  ...
  LEFT JOIN category ON fees.category = category.category_id
  LEFT JOIN user ON fees.author = user.user_id
  WHERE fees.author = {$_SESSION['user_id']}
  ORDER BY fees.fees_id ASC LIMIT {$offset },{$limit}";
}
...
<td class='edit'><a href='update-fees.php?id=<?php echo $row['fees_id'] ?>'><i class='fa fa-edit'></i></a></td>
<td class='delete'><a onclick="return confirm('Are You Sure?')" href='delete-fees.php?id=<?php echo $row['fees_id'] ?>&catid=<?php echo $row['category'] ?>'><i class='fa fa-trash-o'></i></a></td>
...
<?php 
      ...
      if(mysqli_num_rows($result2)){
        ...
        if($page_number > 1){
          echo '<li><a href="fees.php?page='.($page_number-1).'">prev</a></li>';
        }
        ...
        for($i = 1; $i <= $total_page; $i++){
          ...
          echo '<li class='.$active.'><a href="fees.php?page='.$i.'">'.$i.'</a></li>';
        }
        if($total_page > $page_number){
          echo '<li><a href="fees.php?page='.($page_number+1).'">next</a></li>';
        }
        echo "</ul>";
      }
...
?></center> 
			...
			<div class="col-md-10">
                  <h1 class="admin-heading">International Student Fee Structure</h1>
              </div>
              <div class="col-md-2">
                  <a class="add-new" href="add-ifees.php"><button class="btn">Add iFees</button></a>
              </div>  
...
if($_SESSION['user_role'] == '1'){
   $query = "SELECT ifees.ifees_id, ifees.description1, ifees.description2, ifees.description3, ifees.description4, ifees.description5, ifees.description6, ifees.description7, ifees.description8, ifees.description9, ifees.description10,ifees.description11, ifees.description12, ifees.description13, ifees.ifees_date,ifees.category, category.category_name,user.username FROM ifees
  ...
  LEFT JOIN category ON ifees.category = category.category_id
  LEFT JOIN user ON ifees.author = user.user_id
  WHERE ifees.author = {$_SESSION['user_id']}
  ORDER BY ifees.ifees_id ASC LIMIT {$offset },{$limit}";
}
...
?>
                          <tr>
							  ...
							  <td><?php echo $row['description1'] ?></td>
                              <td><?php echo $row['category_name'] ?></td>
                              <td><?php echo $row['ifees_date'] ?></td>
                              <td><?php echo $row['username'] ?></td>

<td class='edit'><a href='update-ifees.php?id=<?php echo $row['ifees_id'] ?>'><i class='fa fa-edit'></i></a></td>
<td class='delete'><a onclick="return confirm('Are You Sure?')" href='delete-ifees.php?id=<?php echo $row['ifees_id'] ?>&catid=<?php echo $row['category'] ?>'><i class='fa fa-trash-o'></i></a></td>
