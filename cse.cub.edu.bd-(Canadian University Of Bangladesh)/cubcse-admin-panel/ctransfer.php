...
<div class="container-fluid">
  ...
  <div id="admin-content">
      <div class="container">
          <div class="row">
              <div class="col-md-10">
                  <h1 class="admin-heading">All Credit Transfer Requirements</h1>
              </div>
              <div class="col-md-2">
                  <a class="add-new" href="add-ctransfer.php"><button class="btn">Add ctransfer</button></a>
              </div>
              <div class="col-md-12">
<br>
<?php 
...
if($_SESSION['user_role'] == '1'){
   $query = "SELECT ctransfer.ctransfer_id, ctransfer.description, ctransfer.ctransfer_date,ctransfer.category, category.category_name,user.username FROM ctransfer
  ...
  LEFT JOIN category ON ctransfer.category = category.category_id
  LEFT JOIN user ON ctransfer.author = user.user_id
  WHERE ctransfer.author = {$_SESSION['user_id']}
  ORDER BY ctransfer.ctransfer_id ASC LIMIT {$offset },{$limit}";
}
...
?>
                          <tr>
                              ...
                              <td><?php echo $row['ctransfer_date'] ?></td>
                              <td><?php echo $row['username'] ?></td>

<td class='edit'><a href='update-ctransfer.php?id=<?php echo $row['ctransfer_id'] ?>'><i class='fa fa-edit'></i></a></td>
<td class='delete'><a onclick="return confirm('Are You Sure?')" href='delete-ctransfer.php?id=<?php echo $row['ctransfer_id'] ?>&catid=<?php echo $row['category'] ?>'><i class='fa fa-trash-o'></i></a></td>
...
<?php 
      ...
      if(mysqli_num_rows($result2)){
        ...
        if($page_number > 1){
          echo '<li><a href="ctransfer.php?page='.($page_number-1).'">prev</a></li>';
        }
        ...
        for($i = 1; $i <= $total_page; $i++){
          ...
          echo '<li class='.$active.'><a href="ctransfer.php?page='.$i.'">'.$i.'</a></li>';
        }
        if($total_page > $page_number){
          echo '<li><a href="ctransfer.php?page='.($page_number+1).'">next</a></li>';
        }
        echo "</ul>";
      }
