...
<div class="container-fluid">
  ...
  <div id="admin-content">
      <div class="container">
          <div class="row">
              <div class="col-md-10">
                  <h1 class="admin-heading">All achievement Lists</h1>
              </div>
              <div class="col-md-2">
                  <a class="add-new" href="add-achievement.php"><button class="btn">Add achievement</button></a>
              </div>
              <div class="col-md-12">
<br>
<?php 
...
if($_SESSION['user_role'] == '1'){
   $query = "SELECT achievement.achievement_id, achievement.title, achievement.description, achievement.link, achievement.achievement_img, achievement.achievement_date,achievement.category, category.category_name,user.username FROM achievement
  ...
  LEFT JOIN category ON achievement.category = category.category_id
  LEFT JOIN user ON achievement.author = user.user_id
  WHERE achievement.author = {$_SESSION['user_id']}
  ORDER BY achievement.achievement_id DESC LIMIT {$offset },{$limit}";
}
...
?>
                          <tr>
                              ...
                              <td><img height="50px" src="upload/<?php echo $row['achievement_img'] ?>"></td>
                              <td><?php echo $row['title'] ?></td>
                              <td><?php echo $row['category_name'] ?></td>
                              <td><?php echo $row['achievement_date'] ?></td>
                              <td><?php echo $row['username'] ?></td>

<td class='edit'><a href='update-achievement.php?id=<?php echo $row['achievement_id'] ?>'><i class='fa fa-edit'></i></a></td>
<td class='delete'><a onclick="return confirm('Are You Sure?')" href='delete-achievement.php?id=<?php echo $row['achievement_id'] ?>&catid=<?php echo $row['category'] ?>'><i class='fa fa-trash-o'></i></a></td>
...
<?php 
      ...
      include "config.php";
      $query2 = "SELECT * FROM achievement";
      ...
      if(mysqli_num_rows($result2)){
        ...
        if($page_number > 1){
          echo '<li><a href="achievement.php?page='.($page_number-1).'">prev</a></li>';
        }
        ...
        for($i = 1; $i <= $total_page; $i++){
          ...
          echo '<li class='.$active.'><a href="achievement.php?page='.$i.'">'.$i.'</a></li>';
        }
        if($total_page > $page_number){
          echo '<li><a href="achievement.php?page='.($page_number+1).'">next</a></li>';
        }
        echo "</ul>";
      }
